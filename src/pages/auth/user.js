...
const UserInformation = () => {
    ...
    const handleFileUpload = (event) => {
        ...
        axios
            .post(
                `${process.env.REACT_APP_API_GATEWAY}Uploader/UploadFile?apikey=${process.env.REACT_APP_API_KEY}`,
                formData
            )
            .then((response) => {
    ...
    const handleSendUsersGift = async (dornaG) => {
        ...
        await axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doupdateapi?func=UpdateUserDornaCoin&dornacoin=${dornaG}&apikey=${process.env.REACT_APP_API_KEY}&userid=6&ccid=${user_id}`
            )
            .then(function (response) {
                navigate('/profile')
            })
    ...
    const handleGiftDornaCoin = async () => {
        await axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=GetFirstEntryGift&apikey=${process.env.REACT_APP_API_KEY}`
            )
            .then(function (response) {
    ...
    const handleUploadUserProfileImage = async () => {
        ...
        await axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/Doupdateapi?func=UploadUserImageprofile&Uploadphoto=${filename}&apikey=${process.env.REACT_APP_API_KEY}&ccid=${user_id}`
            )
            .then(function (response) {
    ...
    return (
        <section class="AZ-login-page AZ-page-container">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        <form action="">
                            ...
                            <div class="AZ-field-group">
                                ...
                                <input
                                    type="text"
                                    onChange={handleInformation('name')}
                                    value={user?.name}
                                />
                            </div>
                            <div class="AZ-field-group">
                                ...
                                <input
                                    type="text"
                                    onChange={handleInformation('family')}
                                    value={user?.family}
                                />
                            </div>
                            <div class="AZ-field-group">
                                ...
                                <input
                                    ...
                                    onChange={handleInformation('University')}
                                />
                            </div>
                            <div class="AZ-field-group">
                                ...
                                <input
                                    type="text"
                                    value={user?.major}
                                    onChange={handleInformation('major')}
                                />
                            </div>
                            <div class="AZ-field-group">
                            ...
                            <div class="AZ-field-group">
                                ...
                                <input
                                    type="text"
                                    value={user.introducerid}
                                    placeholder="کد معرف "
                                    onChange={handleInformation('introducerid')}
                                />
                            </div>
                            <div class="AZ-field-group">
                                ...
                                <input
                                    type="text"
                                    value={user?.Mobile}
                                    onChange={handleInformation('Mobile')}
                                />
                            </div>
                            <button
                                type="submit"
