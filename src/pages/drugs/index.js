...
const Drugs = () => {
    ...
    const getShapes = async () => {
        await axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=GetAllHandyDrugsShapes&apikey=${process.env.REACT_APP_API_KEY}`
            )
    ...
    const getDrugs = async () => {
        await axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=GetAllDrugInfo&apikey=${process.env.REACT_APP_API_KEY}`
            )
            .then(function (response) {
    ...
    const handleSearch = async (value) => {
        ...
        await axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=SearchDrugInfo&onvandaroinfo=${value}&apikey=${process.env.REACT_APP_API_KEY}`
            )
            .then(function (response) {
                setDrugs(() => response?.data)
            })
                                ...
                                <div class="swiper-wrapper">
                                    {shapes?.map((s, i) => {
                                        return (
                                            <div
                                                key={i}
                                                className="swiper-slide me-2"
                                            >
                                                <div
                                                    className={`AZ-product-category ${colors[i]}`}
                                                >
                                                    <p>
                                                        {s.HandydugsShapeShapes}
                                                    </p>
                                                </div>
                                            </div>
                                        )
