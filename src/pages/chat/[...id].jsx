...
export default function SingleChatPage() {
    ...
    const getMessages = async () => {
        await axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=GetAllChatsforChatRoom&apikey=${process.env.REACT_APP_API_KEY}`
            )
    ...
    const handleSendMessage = async (e) => {
        ...
        axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doupdateapi?func=UpdateChatroom&UserId=${id}&UserMessage=${msg}&MessageDate=${date}&MessageTime=${time}&apikey=${process.env.REACT_APP_API_KEY}`
            )
            .then(function (response) {
    ...
    const handleSearchOnThreads = (value) => {
        axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=SearchPagetwo&Message=${value}&apikey=${process.env.REACT_APP_API_KEY}`
            )
            .then(function (response) {
                            ...
                            <div className="chat__conversation-board">
                                {messages?.map((m, i) => {
                                    return (
                                        <div
                                            key={i}
                                            className={`chat__conversation-board__message-container ${m.Id}`}
                                        >
                                            <div className="chat__conversation-board__message__context d-flex flex-column">
                                                <span className="time_masage">
                                                    {m.fullname
                                                        ? m.fullname
                                                        : '---'}
                                                </span>
