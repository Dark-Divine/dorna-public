...
import { useDrugStore } from '../../store/useDrugsStore'
import { useSearchDrugs } from '../../hooks/prescription/useSearchDrugs'
...
const Prescription = () => {
    ...
    //Zustand
    const setDrugs = useDrugStore((state) => state.setDrugs)
    const drugs = useDrugStore((state) => state.drugs)
    const [drugsRes, setDrugsRes] = useState([])
    ...
    //Fetch
    const { data: drugsData, isSuccess } = useSearchDrugs()
    ...
    useEffect(() => {
        if (searchValue.length > 1) {
            let filteredDrugs = drugs.filter((item) =>
                item.Drug.toLowerCase().includes(searchValue.toLowerCase())
            )
            setDrugsRes(filteredDrugs)
        } else {
            setDrugsRes([])
        }
    }, [searchValue])
    ...
    const getAllDiseaseNames = () => {
        axios
            .get(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=FirstPrescribtionPage&apikey=${process.env.REACT_APP_API_KEY}`
            )
    ...
    const handleSearch = async (value) => {
        await axios
            .post(
                `${process.env.REACT_APP_API_GATEWAY}api/doapi?func=searchdrugs&drugname=${value}&apikey=${process.env.REACT_APP_API_KEY}`
            )
            .then(function (response) {
    ...
    const removeTag = (tagName) => {
        ...
        setTags((prevTags) => tags.filter((tag) => tag !== tagName))
    }
                                ...
                                <div class="col-lg-12">
                                    <div class="d-flex align-items-center justify-content-center position-relative py-3 mb-4">
                                        <h2 class="page-header-title text-center">
                                            نسخه ها و نسخه شناسی
                                        </h2>
                                        <HandlePreviousPageButton
                                            overrideMethod={goBack}
                                        />
                                    </div>
                                    {tags.length > 0 && (
                                        <h6
                                    ...
                                    {isSearch ? (
                                        ...
                                        <>
                                            {tags.length > 0 ? (
                                                <>
                                                    <div
                                                        className="d-flex flex-wrap gap-1 justify-content-start"
                                                        style={{
                                                            width: 'fit-content',
                                                        }}
                                                    >
                                                        ...
                                                        <div className="mt-1">
                                                            {searchResults?.map(
                                                                (d, i) => {
                                                                    return (
                                                                        <a
                                                                            href={`/prescription/file/${d.Id}?diseaseName=${d.DiseseName}`}
                                                                            className="file-item d-flex align-items-center gap-3"
                                                                            key={
                                                                                d.Id
                                                                            }
                                                                        >
                                                                            {d.icon && (
                                                                                <div className="file-item-img d-flex align-items-center justify-content-center">
                                                                                    <img
                                                                                        src={`${process.env.REACT_APP_FILEMANAGER}${d.icon}`}
                                                                                        alt=""
                                                                                    />
                                                                                </div>
                                                                            )}
                                                                            <div className="flex-grow-1">
                                                                                <div className="d-flex align-items-center justify-content-between gap-2 mb-1">
                                                                                    ...
                                                                                    <p className="file-item-date d-flex align-items-center gap-2">
                                                                                        {d.presrank
                                                                                            ? d.presrank
                                                                                            : '---'}
                                                ...
                                                <div class="row">
                                                    {names?.map((n, i) => {
                                                        return (
                                                            ...
                                                            >
                                                                <Link
                                                                    to={`/prescription/${n.Id}?diseaseName=${n.Diseasename}`}
                                                                    className="AZ-version-category d-flex align-items-center gap-3 justify-content-center h-100 p-4"
                                                                >
                                                                    <img
                                                                        src={`${process.env.REACT_APP_FILEMANAGER}${n.Icon}`}
                                                                        alt=""
                                                                    />
                                                                    <p className="version-category-text">
                                                                        {
                                                                            n.Diseasename
                                                                        }
                                                                    </p>
